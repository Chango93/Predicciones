...
def should_abstain(prob_1, prob_x, prob_2, gap, config):
    ...
    Criterio A: Gap muy bajo Y probabilidades muy dispersas (sin favorito claro).
    Criterio B: Ningún resultado supera la probabilidad mínima de confianza (40%).
                Ej: 39.8% / 35.5% / 24.8% → apenas mejor que un volado, no pick.
    """
    ...
    # Criterio A: gap pequeño Y distribución equilibrada
    is_tight = gap < threshold
    is_balanced = spread < spread_threshold

    # Criterio B: ningún resultado tiene probabilidad suficiente
